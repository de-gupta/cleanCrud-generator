package ${postCommitHooks().deletePackage()};
...
@Component("${postCommitHooks().deleteQualifier()}")
class ${postCommitHooks().deleteTypeName()} implements PostCommitMutation<Long, ${aggregate().baseName()}DomainModel>
{
	@Override
	public void accept(final PostCommitMutationContext<Long, ${aggregate().baseName()}DomainModel> context)
	{
		// TODO: implement post-commit behavior for delete operations.
	}
}
